...
public sealed class TileMovementSystem : EntitySystem
{
    ...
    private void OnMapInit(Entity<TileMovementComponent> ent, ref MapInitEvent args)
    {
        ...
        StartSlideTo(ent, target.Comp2, Vector2.Zero);
        UpdateSlide((ent, mover, ent.Comp), target);
    }
        ...
        float frameTime)
    {
        ...
        if (wasWeightless)
        {
            StartSlideTo((target, comp), target.Comp2, Vector2.Zero);
            SetButtons((player, comp), MoveButtons.None);
        ...
        TransformComponent xform)
    {
        ...
        StartSlideTo((player, player.Comp2), xform, offset);
    ...
    public void StartSlideTo(
        ...
        Vector2 offset)
    {
        player.Comp.Origin = xform.LocalPosition;
        player.Comp.Destination = SnapCoordinatesToTile(xform.LocalPosition + offset);
        player.Comp.MovementKeyPressedAt = CurrentTime;
        DirtyField(player, player.Comp, nameof(TileMovementComponent.Origin));
        DirtyField(player, player.Comp, nameof(TileMovementComponent.Destination));
