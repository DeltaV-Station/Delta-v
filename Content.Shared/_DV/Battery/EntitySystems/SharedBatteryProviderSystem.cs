...
/// </summary>
public abstract class SharedBatteryProviderSystem : EntitySystem
{
    ...
    private void OnProviderEquipped(Entity<BatteryProviderComponent> provider, ref ClothingGotEquippedEvent args)
    {
        ...
        foreach (var item in _inventory.GetHandOrInventoryEntities(args.Wearer))
        {
            RaiseLocalEvent(item, ref ev);
        }

        // Also inform any augments that would like to draw power from a provider
        if (TryComp<InstalledAugmentsComponent>(args.Wearer, out var installedAugs))
        {
            foreach (var augment in installedAugs.InstalledAugments)
            {
    ...
    protected virtual void OnProviderUnequipped(Entity<BatteryProviderComponent> provider, ref ClothingGotUnequippedEvent args)
    {
        ...
        // Inform equipment and items that the provider is gone.
        ...
        // Also inform any augments that would have drawn power from a provider
        if (TryComp<InstalledAugmentsComponent>(args.Wearer, out var installedAugs))
        {
            foreach (var augment in installedAugs.InstalledAugments)
            {
