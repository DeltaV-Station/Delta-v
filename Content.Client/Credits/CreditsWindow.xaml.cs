...
public sealed partial class CreditsWindow : DefaultWindow
{
    ...
    private void PopulatePatrons(BoxContainer patronsContainer)
    {
        ...
        var first = true;
        foreach (var tier in patrons.Where(p => p.Show).GroupBy(p => p.Tier).OrderBy(p => PatronTierPriority[p.Key]))
        {
            if (!first)
    ...
    private IEnumerable<PatronEntry> LoadPatrons()
    {
        ...
        return sequence
            ...
            .Select(m => new PatronEntry(m["Name"].AsString(), m["Tier"].AsString(), m["Show"].AsBool()));
    }
    ...
    private sealed class PatronEntry
    {
        ...
        public bool Show {  get; } // Delta V - Boolean to allow hiding Attribution

        public PatronEntry(string name, string tier, bool show)
        {
            Name = name;
            Tier = tier;
            Show = show; // Delta V - Boolean to allow hiding Attribution
        }
    }
}
