...
public sealed partial class DVCommunicationsConsoleMenu : FancyWindow
{
    ...
    public void Update(Entity<DVCommunicationsConsoleComponent> console, EntityUid user)
    {
        ...
        // If they only have the ability to announce, there's no point in having a home button
        ...
        // Station
        StationHeaderLabel.Visible = console.Comp.CanAnnounce || console.Comp.CanConfigureScreens || console.Comp.CanAlertLevel;
        ...
        // Shuttles
        TransitHeaderLabel.Visible = console.Comp.CanCallShuttles; // Should be only hidden if all buttons under are also hidden.
        ShuttlesTab.Visible = console.Comp.CanCallShuttles;

        // Emergency
        EmergencyHeaderLabel.Visible = console.Comp.CanKeycardAuthenticationDevice; // Should be only hidden if all buttons under are also hidden.
    ...
    private void UpdateKeycardAuthenticationDevice()
    {
        ...
        var canCall = !_entity.HasComponent<DVStationKeycardAuthenticationDeviceAlreadySwipedComponent>(user)
                      && _entity.HasComponent<HumanoidProfileComponent>(user) // Can't be AI or a borg
        ...
        if (CallZeta.IsConfirming || RequestCodes.IsConfirming)
            return;

        // Kinda messy but it works
        if (station.Comp.SwipesStartedAt is { } firstSwipeTime)
        {
            var countdown = firstSwipeTime + station.Comp.SwipeWindow;
            var timeRemaining = countdown.Subtract(_timing.CurTime);
            if (timeRemaining > TimeSpan.Zero && station.Comp.SwipingFor is DVStationKeycardAction.Mayday)
                CallZeta.Text = $"{timeRemaining:ss\\.fff}";

            if (timeRemaining > TimeSpan.Zero && station.Comp.SwipingFor is DVStationKeycardAction.Scuttling)
                RequestCodes.Text = $"{timeRemaining:ss\\.fff}";
        }
        else
        {
            CallZeta.Text = Loc.GetString("comms-console-menu-declare-mayday.button");
            RequestCodes.Text = Loc.GetString("comms-console-menu-request-codes.button");
        }
    }
