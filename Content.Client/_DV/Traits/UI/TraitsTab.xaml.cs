...
public sealed partial class TraitsTab : BoxContainer
{
    ...
    public void RefreshTraits()
    {
        ...
        PopulateCategories();
        SetSelectedTraits(previouslySelected);
    }

    private HashSet<string> GetTraitIdsFromYaml()
    {
        var ids = new HashSet<string>();

        var files = _resourceManager.ContentFindFiles(new ResPath("/Prototypes"))
        ...
        foreach (var file in files)
        {
            ...
            try
            {
                yamlStream = _resourceManager.ContentFileReadYaml(file);
            }
            catch
            {
                continue;
            ...
            foreach (var document in yamlStream.Documents)
            {
                ...
                foreach (var node in sequence.Children)
                {
                    ...
                    if (!mapping.TryGetNode("type", out YamlScalarNode? typeNode) || typeNode.Value != "trait")
                        continue;

                    if (mapping.TryGetNode("id", out YamlScalarNode? idNode) && idNode.Value is { } id)
                        ids.Add(id);
                }
            }
        }
    ...
    private void PopulateCategories()
    {
        ...
        var existingTraitIds = GetTraitIdsFromYaml();

        var traitsByCategory = _prototype.EnumeratePrototypes<TraitPrototype>()
            .Where(t => existingTraitIds.Contains(t.ID))
            .GroupBy(t => t.ID)
            .Select(g => g.Last())
    ...
    public void SetSelectedTraits(IEnumerable<ProtoId<TraitPrototype>> traits)
    {
        ...
        foreach (var traitId in traits)
        {
            ...
            if (!_categoryUis.TryGetValue(trait.Category, out var categoryUi) || !categoryUi.ContainsTrait(traitId))
                continue;
