...
public sealed partial class MicrowaveSystem : EntitySystem
{
    ...
    public void TryStartAssembly(EntityUid uid, MicrowaveComponent component, AssemblerStartCookMessage args)
    {
        ...
        foreach (var item in component.Storage.ContainedEntities.ToArray())
        {
            var ev = new BeingMicrowavedEvent(uid, user, component.CurrentCookTimerTime,component.CanHeat, component.CanIrradiate);
        ...
        activeComp.CookTimeRemaining = component.CurrentCookTimerTime; // DeltaV - we dont use FinalCookTimeMultiplier cause we dont have upgradable machine parts
        activeComp.TotalTime = component.CurrentCookTimerTime; //this doesn't scale so that we can have the "actual" time
        activeComp.PortionedRecipe = portionedRecipe;
        //Scale times with cook times
        component.CurrentCookTimeEnd = _gameTiming.CurTime + TimeSpan.FromSeconds(component.CurrentCookTimerTime); // DeltaV - we dont use FinalCookTimeMultiplier cause we dont have upgradable machine parts
        if (malfunctioning)
