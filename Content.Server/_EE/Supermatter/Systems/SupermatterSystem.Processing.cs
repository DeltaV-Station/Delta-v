...
public sealed partial class SupermatterSystem
{
    ...
    private void ProcessAtmos(EntityUid uid, SupermatterComponent sm, float frameTime)
    {
        ...
        var gasEfficiency = sm.GasEfficiency / (sm.Power > 0 ? 1 : _config.GetCVar(EECVars.SupermatterGasEfficiencyGraceModifier));
        ...
        if (ammoniaProportion > 0)
        {
            ...
            var consumedMiasma = Math.Clamp((ammoniaPartialPressure - _config.GetCVar(EECVars.SupermatterAmmoniaConsumptionPressure)) /
                (ammoniaPartialPressure + _config.GetCVar(EECVars.SupermatterAmmoniaPressureScaling)) *
                (1 + powerRatio * _config.GetCVar(EECVars.SupermatterAmmoniaGasMixScaling)),
                0f, 1f);
            ...
            if (consumedMiasma > 0)
            {
                ...
                sm.MatterPower += consumedMiasma * _config.GetCVar(EECVars.SupermatterAmmoniaPowerGain);
            }
        }
        ...
        sm.MoleHeatPenaltyThreshold = (float)Math.Max(moles / _config.GetCVar(EECVars.SupermatterMoleHeatPenalty), 0.25);

        // Ramps up or down in increments of 0.02 up to the proportion of CO2
        // Given infinite time, powerloss_dynamic_scaling = co2comp
        // Some value from 0-1
        if (moles > _config.GetCVar(EECVars.SupermatterPowerlossInhibitionMoleThreshold) &&
            gasComposition.GetMoles(Gas.CarbonDioxide) > _config.GetCVar(EECVars.SupermatterPowerlossInhibitionGasThreshold))
        {
        ...
        sm.PowerlossInhibitor = Math.Clamp(
            1 - sm.PowerlossDynamicScaling * Math.Clamp(moles / _config.GetCVar(EECVars.SupermatterPowerlossInhibitionMoleBoostThreshold), 1f, 1.5f),
            0f, 1f);

        if (sm.MatterPower != 0)
        {
            // We base our removed power off 1/10 the matter_power.
            var removedMatter = Math.Max(sm.MatterPower / _config.GetCVar(EECVars.SupermatterMatterPowerConversion), 40);
        ...
        {
            rad.Intensity =
                _config.GetCVar(EECVars.SupermatterRadsBase) +
                sm.Power
                * Math.Max(0, 1f + transmissionBonus / 10f)
                * 0.003f
                * _config.GetCVar(EECVars.SupermatterRadsModifier);
        ...
        var energy = sm.Power * _config.GetCVar(EECVars.SupermatterReactionPowerModifier) * (1f - sm.PsyCoefficient * 0.2f) * 2 * frameTime;
        ...
        gasReleased.Temperature += energy * sm.HeatModifier / _config.GetCVar(EECVars.SupermatterThermalReleaseModifier);
        ...
        gasReleased.AdjustMoles(
            Gas.Plasma,
            Math.Max(energy * sm.HeatModifier / _config.GetCVar(EECVars.SupermatterPlasmaReleaseModifier), 0f));
        gasReleased.AdjustMoles(
            Gas.Oxygen,
            Math.Max((energy + gasReleased.Temperature * sm.HeatModifier - Atmospherics.T0C) / _config.GetCVar(EECVars.SupermatterOxygenReleaseModifier), 0f));
    ...
    private void SupermatterZap(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.Power >= _config.GetCVar(EECVars.SupermatterPowerPenaltyThreshold))
            zapCount += 2;

        if (sm.Power >= _config.GetCVar(EECVars.SupermatterSeverePowerPenaltyThreshold))
        {
            zapPower += 1;
            zapCount += 1;
        }

        if (sm.Power >= _config.GetCVar(EECVars.SupermatterCriticalPowerPenaltyThreshold))
        {
            zapPower += 1;
            zapCount += 1;
        }
    ...
    private void GenerateAnomalies(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.Power > _config.GetCVar(EECVars.SupermatterSeverePowerPenaltyThreshold) && _random.Prob(1 / sm.AnomalyGravityChanceSevere) ||
        ...
        if (sm.Power > _config.GetCVar(EECVars.SupermatterSeverePowerPenaltyThreshold) && _random.Prob(1 / sm.AnomalyPyroChanceSevere) ||
            sm.Power > _config.GetCVar(EECVars.SupermatterPowerPenaltyThreshold) && _random.Prob(1 / sm.AnomalyPyroChance))
    ...
    private void HandleDamage(EntityUid uid, SupermatterComponent sm)
    {
        ...
        var tempThreshold = Atmospherics.T0C + _config.GetCVar(EECVars.SupermatterHeatPenaltyThreshold);
        ...
        var powerDamage = Math.Max(sm.Power - _config.GetCVar(EECVars.SupermatterPowerPenaltyThreshold), 0f) / 500f * sm.DamageIncreaseMultiplier;
        totalDamage += powerDamage;

        // Mol count only starts affecting damage when it is above 1800
        var moleDamage = Math.Max(moles - _config.GetCVar(EECVars.SupermatterMolePenaltyThreshold), 0f) / 80 * sm.DamageIncreaseMultiplier;
        totalDamage += moleDamage;

        // Healing damage
        if (moles < _config.GetCVar(EECVars.SupermatterMolePenaltyThreshold))
        {
    ...
    private void AnnounceCoreDamage(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.Damage < sm.DamageDelaminationPoint && sm.DelamAnnounced)
        {
            ...
            sm.YellTimer = TimeSpan.FromSeconds(_config.GetCVar(EECVars.SupermatterYellTimer));
            global = true;
        ...
        if (sm.Delamming && sm.DelamAnnounced)
        {
            ...
            sm.YellTimer = seconds switch
            {
                ...
                _ => TimeSpan.FromSeconds(_config.GetCVar(EECVars.SupermatterYellTimer))
            };
        ...
        if (sm.Damage >= sm.DamageWarningThreshold)
        {
            ...
            if (sm.GasStorage != null && sm.GasStorage.TotalMoles >= _config.GetCVar(EECVars.SupermatterMolePenaltyThreshold))
            {
    ...
    {
        if (_config.GetCVar(EECVars.SupermatterDoForceDelam))
            return _config.GetCVar(EECVars.SupermatterForcedDelamType);
        ...
        if (mix is { })
        {
            ...
            if (_config.GetCVar(EECVars.SupermatterDoSingulooseDelam)
                && moles >= _config.GetCVar(EECVars.SupermatterMolePenaltyThreshold) * _config.GetCVar(EECVars.SupermatterSingulooseMolesModifier))
                return DelamType.Singulo;
        }

        if (_config.GetCVar(EECVars.SupermatterDoTeslooseDelam)
            && sm.Power >= _config.GetCVar(EECVars.SupermatterPowerPenaltyThreshold) * _config.GetCVar(EECVars.SupermatterTesloosePowerModifier))
