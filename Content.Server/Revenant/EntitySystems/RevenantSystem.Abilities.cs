...
public sealed partial class RevenantSystem
{
    ...
    [Dependency] private readonly SharedAudioSystem _audioSystem = default!; // End Imp Changes
    ...
    [ValidatePrototypeId<StatusEffectPrototype>] // Begin Imp Changes
    ...
    private void OnHauntAction(EntityUid uid, RevenantComponent comp, RevenantHauntActionEvent args)
    {
        ...
        var witnessAndRevenantFilter = Filter.Pvs(uid).RemoveWhere(player =>
        {
            ...
            if (!HasComp<MobStateComponent>(ent) || !HasComp<HumanoidProfileComponent>(ent) || HasComp<RevenantComponent>(ent))
                return true;
