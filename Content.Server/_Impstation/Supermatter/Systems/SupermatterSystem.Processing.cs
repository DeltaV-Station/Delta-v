...
public sealed partial class SupermatterSystem
{
    ...
    private void ProcessAtmos(EntityUid uid, SupermatterComponent sm, float frameTime)
    {
        ...
        if (ammoniaProportion > 0)
        {
            ...
            var consumedMiasma = Math.Clamp((ammoniaPartialPressure - _config.GetCVar(ImpCCVars.SupermatterAmmoniaConsumptionPressure)) /
                (ammoniaPartialPressure + _config.GetCVar(ImpCCVars.SupermatterAmmoniaPressureScaling)) *
                (1 + powerRatio * _config.GetCVar(ImpCCVars.SupermatterAmmoniaGasMixScaling)),
                0f, 1f);
        ...
        sm.MoleHeatPenaltyThreshold = (float)Math.Max(moles / _config.GetCVar(ImpCCVars.SupermatterMoleHeatPenalty), 0.25);

        // Ramps up or down in increments of 0.02 up to the proportion of CO2
        // Given infinite time, powerloss_dynamic_scaling = co2comp
        // Some value from 0-1
        if (moles > _config.GetCVar(ImpCCVars.SupermatterPowerlossInhibitionMoleThreshold) &&
            gasComposition.GetMoles(Gas.CarbonDioxide) > _config.GetCVar(ImpCCVars.SupermatterPowerlossInhibitionGasThreshold))
        {
        ...
        sm.PowerlossInhibitor = Math.Clamp(
            1 - sm.PowerlossDynamicScaling * Math.Clamp(moles / _config.GetCVar(ImpCCVars.SupermatterPowerlossInhibitionMoleBoostThreshold), 1f, 1.5f),
            0f, 1f);

        if (sm.MatterPower != 0)
        {
            // We base our removed power off 1/10 the matter_power.
            var removedMatter = Math.Max(sm.MatterPower / _config.GetCVar(ImpCCVars.SupermatterMatterPowerConversion), 40);
        ...
        if (TryComp<RadiationSourceComponent>(uid, out var rad))
        {
            rad.Intensity =
                ...
                * 0.003f
                * _config.GetCVar(ImpCCVars.SupermatterRadsModifier);
        ...
        gasReleased.AdjustMoles(
            Gas.Plasma,
            Math.Max(energy * sm.HeatModifier / _config.GetCVar(ImpCCVars.SupermatterPlasmaReleaseModifier), 0f));
        gasReleased.AdjustMoles(
            Gas.Oxygen,
            Math.Max((energy + gasReleased.Temperature * sm.HeatModifier - Atmospherics.T0C) / _config.GetCVar(ImpCCVars.SupermatterOxygenReleaseModifier), 0f));
    ...
    private void GenerateAnomalies(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.Power > _config.GetCVar(ImpCCVars.SupermatterSeverePowerPenaltyThreshold) && _random.Prob(1 / sm.AnomalyGravityChanceSevere) ||
        ...
        if (sm.Power > _config.GetCVar(ImpCCVars.SupermatterSeverePowerPenaltyThreshold) && _random.Prob(1 / sm.AnomalyPyroChanceSevere) ||
            sm.Power > _config.GetCVar(ImpCCVars.SupermatterPowerPenaltyThreshold) && _random.Prob(1 / sm.AnomalyPyroChance))
    ...
    private void HandleDamage(EntityUid uid, SupermatterComponent sm)
    {
        ...
        var powerDamage = Math.Max(sm.Power - _config.GetCVar(ImpCCVars.SupermatterPowerPenaltyThreshold), 0f) / 500f * sm.DamageIncreaseMultiplier;
        totalDamage += powerDamage;

        // Mol count only starts affecting damage when it is above 1800
        var moleDamage = Math.Max(moles - _config.GetCVar(ImpCCVars.SupermatterMolePenaltyThreshold), 0f) / 80 * sm.DamageIncreaseMultiplier;
