...
public sealed class GravityWellSystem : SharedGravityWellSystem
{
    ...
    public override void Initialize()
    {
        ...
        vvHandle.AddPath(nameof(GravityWellComponent.TargetPulsePeriod),
        ...
        TransformComponent? xform = null)
    {
        ...
        GravPulse(uid,
            gravWell.MaxRange,
            gravWell.MinRange,
            gravWell.BaseRadialAcceleration * scale,
            gravWell.BaseTangentialAcceleration * scale,
            xform);
    }
    ...
    public void GravPulse(EntityUid uid,
        float maxRange,
        float minRange,
        in Matrix3x2 baseMatrixDeltaV,
        TransformComponent? xform = null)
    {
    ...
    public void GravPulse(EntityUid uid,
        float maxRange,
        float minRange,
        float baseRadialDeltaV = 0.0f,
        float baseTangentialDeltaV = 0.0f,
        TransformComponent? xform = null)
    {
        if (Resolve(uid, ref xform))
    ...
    /// <param name="baseRadialDeltaV">The base radial velocity that will be added to entities within range towards the center of the gravitational pulse.</param>
    /// <param name="baseTangentialDeltaV">The base tangential velocity that will be added to entities within countrclockwise around the center of the gravitational pulse.</param>
    public void GravPulse(EntityCoordinates entityPos,
        float maxRange,
        float minRange,
        float baseRadialDeltaV = 0.0f,
        float baseTangentialDeltaV = 0.0f)
        => GravPulse(_transform.ToMapCoordinates(entityPos),
            maxRange,
            minRange,
            baseRadialDeltaV,
    ...
    public void GravPulse(MapCoordinates mapPos, float maxRange, float minRange, in Matrix3x2 baseMatrixDeltaV)
    {
        ...
        var minRange2 =
            MathF.Max(minRange * minRange,
                MinGravPulseRange); // Cache square value for speed. Also apply a sane minimum value to the minimum value so that div/0s don't happen.
        ...
        foreach (var entity in _entSet)
        {
            ...
            if (TryComp<MovedByPressureComponent>(entity, out var movedPressure) &&
                !movedPressure.Enabled) //Ignore magboots users
                continue;
            ...
            _physics.ApplyLinearImpulse(entity,
    ...
    public void GravPulse(MapCoordinates mapPos,
        float maxRange,
        float minRange = 0.0f,
        float baseRadialDeltaV = 0.0f,
        float baseTangentialDeltaV = 0.0f)
        => GravPulse(mapPos,
            maxRange,
            minRange,
            new Matrix3x2(baseRadialDeltaV, -baseTangentialDeltaV, baseTangentialDeltaV, baseRadialDeltaV, 0.0f, 0.0f));
